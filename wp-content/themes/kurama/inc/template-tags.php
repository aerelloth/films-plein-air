...
function kurama_paging_nav() {
	...
	<nav class="navigation paging-navigation" role="navigation">
		...
		<h1 class="screen-reader-text"><?php _e( 'Navigation des posts', 'kurama' ); ?></h1>
		...
		<div class="nav-links">
			...
			<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Posts plus anciens', 'kurama' ) ); ?></div>
			...
			<div class="nav-next"><?php previous_posts_link( __( 'Posts plus récents <span class="meta-nav">&rarr;</span>', 'kurama' ) ); ?></div>
	...
	<nav class="navigation post-navigation" role="navigation">
		...
		<div class="nav-links">
			...
			<?php
				...
				previous_post_link( '<div class="nav-previous">%link</div>', _x( '<span class="meta-nav">&larr;</span>&nbsp;%title', 'Vers le post précédent', 'kurama' ) );

				next_post_link(     '<div class="nav-next">%link</div>',     _x( '%title&nbsp;<span class="meta-nav">&rarr;</span>', 'Vers le post suivant',     'kurama' ) );
...
function kurama_posted_on() {
	...
	$posted_on = sprintf(
		...
		_x( 'Posté le %s', 'post date', 'kurama' ),
...
function kurama_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		...
		if ( $categories_list && kurama_categorized_blog() ) {
			...
			printf( '<span class="cat-links">' . __( 'Posté dans %1$s', 'kurama' ) . '</span>', $categories_list );
		...
		if ( $tags_list ) {
			...
			printf( '<span class="tags-links">' . __( 'Etiquettes : %1$s', 'kurama' ) . '</span>', $tags_list );
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( __( 'Laisser un commentaire', 'kurama' ), __( '1 Commentaire', 'kurama' ), __( '% Commentaires', 'kurama' ) );
	...
	edit_post_link( __( 'Modifier', 'kurama' ), '<span class="edit-link">', '</span>' );
...
function kurama_comment( $comment, $args, $depth ) {
	...
	<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
		...
		<div class="comment-body">
			...
			<?php _e( 'Pingback:', 'kurama' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Modifier', 'kurama' ), '<span class="edit-link">', '</span>' ); ?>
	...
	<li id="comment-<?php comment_ID(); ?>" <?php comment_class( empty( $args['has_children'] ) ? '' : 'parent' ); ?>>
		...
		<article id="div-comment-<?php comment_ID(); ?>" class="comment-body row">
			...
			<footer class="comment-meta">
				...
				<p class="comment-awaiting-moderation"><?php _e( 'Votre commentaire est en attente de modération.', 'kurama' ); ?></p>
			...
			<?php edit_comment_link( __( 'Modifier', 'kurama' ), '<span class="edit-link">', '</span>' ); ?>
